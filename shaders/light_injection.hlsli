...
PS_OUT PS_MAIN(VS_TO_PS pixel_input)
{
	...
	float2 t = R16G16_SNORM_to_FLOAT2(1670093964);

	uint b = R16G16_FLOAT2_to_SNORM(t);

	float2 a = R16G16_SNORM_to_FLOAT2(b);

	PS_OUT pixel_output;
	pixel_output.Color = float4(a.x, a.y, 0.0, 1.0) + float4(light_injection_mesh_material_set_constant_buffer.g_base_color, 1.0);
