...
extern renderer *renderer_init(void *wsi_connection)
{
	renderer *new_renderer = new (malloc(sizeof(renderer))) renderer{};
	new_renderer->init(wsi_connection);
	return new_renderer;
}
...
void renderer::init(void *wsi_connection)
{
	this->m_device = brx_init_unknown_device(wsi_connection, true);
	...
	// Demo Init
	this->m_demo.init(this->m_device);
